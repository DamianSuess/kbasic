												...
												)
{
  ...
  //int n2 = strlen(acRealName);
  //int n = strlen(acName) - n2;
  n -= n2;
																 ...
																 )
{
    ...
    bool bIsCached = nSubId != 0;

		nIdCounter++;
		if (nSubId == 0) nSubId = nIdCounter; else nIdCounter = nSubId;
    ...
    c->bCached = bIsCached;
    list_sub.add(nSubId, (int) c);
		list_sub2.addString(getKey(acModule, acName), (int) c);
		list_sub3.addString(getKeyFromRealNameAndFileAndLine(acModule, acRealName, acFile, nLine), (int) c);
		list_sub4.add(nModuleId, (int) c);
...
char     *memory_sub::getModuleByModuleId			(int nModuleId)
{
	if (list_sub4.contains(nModuleId)){
		sub *m = (sub *) list_sub4.get(nModuleId);
   	if (m != 0){
	  	return m->acModule;
	  }
	}	 
...
{
  return getScope(nId) == token__PUBLIC;
}

bool     memory_sub::isPrivate			(int nId)
{
  return getScope(nId) == token__PRIVATE;
}

bool     memory_sub::isProtected			(int nId)
{
  return getScope(nId) == token__PROTECTED;
}
...
int			memory_sub::getIdFromRealNameAndFileAndLine					(const char *acModule, const char *acRealName, const char *acFile, int nLine)
{
  char *acKey = getKeyFromRealNameAndFileAndLine(acModule, acRealName, acFile, nLine);
    ...
    return ((sub *)list_sub3.getString(acKey))->nId;
	}	 
...
int			memory_sub::getId					(const char *acModule, const char *acName)
{
  char *acKey = getKey(acModule, acName);
    ...
    return ((sub *)list_sub2.getString(acKey))->nId;
	}	 
...
bool		memory_sub::addThrow(int nId, const char *acType)
{
    ...
    for (int i = 0; i < m->nThrowCounter; i++){
      if (utility::my_stricmp(m->acThrowType[i], acType) == 0) return false;
    }
...
bool		memory_sub::hasThrow	(int nId, const char *acType)
{
    ...
    if (firstThrow(nId, &ac)){
      do {
        ...
        if (utility::my_stricmp(ac, acType) == 0){ return true; }
...
bool		memory_sub::addParameter(int nId, const char *acName, const char *acType, int nParameterCall, bool bOptional, bool bDefault, bool bParamArray, t_array *a, defaultvalue *defval, int *nParamId, int *nParamLStringLen)
{
	sub *m = findSub	(nId);
	if (m != 0){
    ...
    // acName must be unique
      ...
      if (*nParamId == 0){ 
        *nParamId = m->nParameterParamId[m->nParameterCounter] = memory_variable::addIdCounter();
        ...
        memory_variable::setIdCounter(*nParamId);
        m->nParameterParamId[m->nParameterCounter] = *nParamId;
      }

      m->nParameterLStringLen[m->nParameterCounter] = *nParamLStringLen;
...
bool		memory_sub::firstParameter	(int nId, const char **acName, const char **acType, int *nParameterCall, bool *bOptional, bool *bDefault, bool *bParamArray, t_array **a, defaultvalue **defval, int *nParamId, int *nParamLStringLen)
{
	sub *m = findSub	(nId);
	if (m != 0){
      ...
      *nParamId = m->nParameterParamId[m->nParameterIterator];
			*nParamLStringLen = m->nParameterLStringLen[m->nParameterIterator];      
...
bool		memory_sub::nextParameter	(int nId, const char **acName, const char **acType, int *nParameterCall, bool *bOptional, bool *bDefault, bool *bParamArray, t_array **a, defaultvalue **defval, int *nParamId, int *nParamLStringLen)
{
	sub *m = findSub	(nId);
	if (m != 0){
...
bool		memory_sub::lastParameter	(int nId, const char **acName, const char **acType, int *nParameterCall, bool *bOptional, bool *bDefault, bool *bParamArray, t_array **a, defaultvalue **defval, int *nParamId, int *nParamLStringLen)
{
	sub *m = findSub	(nId);
	if (m != 0){
...
bool		memory_sub::prevParameter	(int nId, const char **acName, const char **acType, int *nParameterCall, bool *bOptional, bool *bDefault, bool *bParamArray, t_array **a, defaultvalue **defval, int *nParamId, int *nParamLStringLen)
{
	sub *m = findSub	(nId);
	if (m != 0){
...
bool		memory_sub::getParameter	(int nId, const char *acName_, const char **acType, int *nParameterCall, bool *bOptional, bool *bDefault, bool *bParamArray, t_array **a, defaultvalue **defval, int *nParamId, int *nParamLStringLen)
{	
  const char *acName = "";

  if (firstParameter (nId, &acName, acType, nParameterCall, bOptional, bDefault, bParamArray, a, defval, nParamId, nParamLStringLen)){

		do{ 
			if (utility::my_stricmp(acName_, acName) == 0){
				return true;
			}

		} while(nextParameter (nId, &acName, acType, nParameterCall, bOptional, bDefault, bParamArray, a, defval, nParamId, nParamLStringLen));
...
bool		memory_sub::findParameter	(int nSubId, int nId, const char **acModuleName, const char **acSub, const char **acName)
{
  ...
  if (firstParameter (nSubId, &_acName, &acType, &nParameterCall, &bOptional, &bDefault, &bParamArray, &a, &defval, &nParamId, &nParamLStringLen)){
		...
		} while(nextParameter (nSubId, &_acName, &acType, &nParameterCall, &bOptional, &bDefault, &bParamArray, &a, &defval, &nParamId, &nParamLStringLen));
...
void memory_sub::registerCall(int nId, pcode *pcodeCall, int nPcodePos)//, int nReturnPcodePos)
{
	...
	if (m != 0){
		...
		m->bNotChanged[m->nPcodeCallCounter] = true;
    if (m->pcodeCall[m->nPcodeCallCounter] == 0){
      m = m;
    }
    ...
    do {
      int nId = getId();
        ...
        for (int i = 0; i < m->nPcodeCallCounter; i++){
          if (/*m->bNotChanged[i] && */m->pcodeCall[i] == p_old){
...
{
  idx = _idx;
  return list_sub.first(&key, (int *) &it[idx]);
}

bool		memory_sub::next	(int _idx)
{
  idx = _idx;
  return list_sub.next(&key, (int *) &it[idx]);
}
...
int		memory_sub::addVar(int nModuleId, const char *acSub, const char *acName, int nType, t_array *a, bool bStatic, int nScope, int nLStringLen, int nId)
{
    ...
    return m->vars->declare(m->acModule, acSub, acName, nType, a, bByVal, bReference, bStatic, nScope, nLStringLen, nId);
...
int memory_sub::getLStringLenVar		(const char *acModule, const char *acSub, const char *acName)
{
	sub *m = findSub	(getId(acModule, acSub));
	if (m != 0){

		return m->vars->getLStringLen(m->vars->getId(acModule, acSub, acName));

	}
	return false;
}

int memory_sub::getLStringLenVar		(const char *acModule, const char *acSub, int nIdVar)
{
	sub *m = findSub	(getId(acModule, acSub));
	if (m != 0){

		return m->vars->getLStringLen(nIdVar);
