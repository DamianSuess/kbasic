...
void EVENT_OnTick(t_long MSec){ 
  if (nEVENT_OnTick != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_long));
  if ((nEVENT_OnTick == 2) || (nEVENT_OnTick == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnTick", acGroup, acName, param))){
    nEVENT_OnTick = 2;
		myInterpreter->pushStack(new interpreter_parameter(MSec));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnTick", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnTick = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnTick(MSec);
#endif  
 }

void EVENT_OnStateChanged(QString NewState, QString OldState){ 
  if (nEVENT_OnStateChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__QString)); param->append((pcode__QString));
  if ((nEVENT_OnStateChanged == 2) || (nEVENT_OnStateChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnStateChanged", acGroup, acName, param))){
    nEVENT_OnStateChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(NewState));
		myInterpreter->pushStack(new interpreter_parameter(OldState));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnStateChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnStateChanged = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnStateChanged(NewState, OldState);
#endif  
 }



void EVENT_OnAboutToFinish(){ 
  if (nEVENT_OnAboutToFinish != 1){
	QList<int> *param = new QList<int>; 
	
  if ((nEVENT_OnAboutToFinish == 2) || (nEVENT_OnAboutToFinish == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnAboutToFinish", acGroup, acName, param))){
    nEVENT_OnAboutToFinish = 2;
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnAboutToFinish", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnAboutToFinish = 1;
}
	CLASS::EVENT_OnAboutToFinish(); 
 }

void EVENT_OnFinished(){ 
  if (nEVENT_OnFinished != 1){
	QList<int> *param = new QList<int>; 
	
  if ((nEVENT_OnFinished == 2) || (nEVENT_OnFinished == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnFinished", acGroup, acName, param))){
    nEVENT_OnFinished = 2;
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnFinished", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnFinished = 1;
}
	CLASS::EVENT_OnFinished(); 
 }

void EVENT_OnMutedChanged(t_boolean OnOff){ 
  if (nEVENT_OnMutedChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_boolean));
  if ((nEVENT_OnMutedChanged == 2) || (nEVENT_OnMutedChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnMutedChanged", acGroup, acName, param))){
    nEVENT_OnMutedChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(OnOff));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnMutedChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnMutedChanged = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnMutedChanged(OnOff);
#endif  
 }

void EVENT_OnVolumeChanged(t_integer Percent){ 
  if (nEVENT_OnVolumeChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_integer));
  if ((nEVENT_OnVolumeChanged == 2) || (nEVENT_OnVolumeChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnVolumeChanged", acGroup, acName, param))){
    nEVENT_OnVolumeChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(Percent));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnVolumeChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnVolumeChanged = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnVolumeChanged(Percent);
#endif  
 }

void EVENT_OnPlayingTitleChanged(QString FileName){ 
  if (nEVENT_OnPlayingTitleChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__QString));
  if ((nEVENT_OnPlayingTitleChanged == 2) || (nEVENT_OnPlayingTitleChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnPlayingTitleChanged", acGroup, acName, param))){
    nEVENT_OnPlayingTitleChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(FileName));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnPlayingTitleChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnPlayingTitleChanged = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnPlayingTitleChanged(FileName);
#endif  
 }

void EVENT_OnBufferStatus(t_integer Percent){ 
  if (nEVENT_OnBufferStatus != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_integer));
  if ((nEVENT_OnBufferStatus == 2) || (nEVENT_OnBufferStatus == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnBufferStatus", acGroup, acName, param))){
    nEVENT_OnBufferStatus = 2;
		myInterpreter->pushStack(new interpreter_parameter(Percent));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnBufferStatus", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnBufferStatus = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnBufferStatus(Percent);
#endif  
 }

void EVENT_OnTotalTimeChanged(t_long MSec){ 
  if (nEVENT_OnTotalTimeChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_long));
  if ((nEVENT_OnTotalTimeChanged == 2) || (nEVENT_OnTotalTimeChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnTotalTimeChanged", acGroup, acName, param))){
    nEVENT_OnTotalTimeChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(MSec));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnTotalTimeChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnTotalTimeChanged = 1;
}
#ifdef CLASS
   CLASS::EVENT_OnTotalTimeChanged(MSec);
#endif  
 }
...
int nEVENT_OnEvent;
int nEVENT_OnTick;
int nEVENT_OnStateChanged;
int nEVENT_OnAboutToFinish;
int nEVENT_OnFinished;
int nEVENT_OnMutedChanged;
int nEVENT_OnVolumeChanged;
int nEVENT_OnPlayingTitleChanged;
int nEVENT_OnBufferStatus;
int nEVENT_OnTotalTimeChanged;
...
{  
  ...
  nEVENT_OnEvent = 0;
  nEVENT_OnTick = 0;
  nEVENT_OnStateChanged = 0;
  nEVENT_OnFinished = 0;
  nEVENT_OnMutedChanged = 0;
  nEVENT_OnVolumeChanged = 0;
  nEVENT_OnPlayingTitleChanged = 0;
  nEVENT_OnBufferStatus = 0;
  nEVENT_OnTotalTimeChanged = 0;
...
/*
void EVENT_OnContextMenu(t_integer X, t_integer Y, t_integer GlobalX, t_integer GlobalY){ 
...
#ifdef CLASS
	CLASS::EVENT_OnContextMenu(X, Y, GlobalX, GlobalY);
#endif	
 }
void EVENT_OnDragEnter(t_boolean *Cancel, t_integer X, t_integer Y, t_integer Width, t_integer Height){ 
  if (nEVENT_OnDragEnter != 1){
	QList<int> *param = new QList<int>;
	 param->append((pcode__t_boolean));  param->append((pcode__t_integer)); param->append((pcode__t_integer)); param->append((pcode__t_integer));  param->append((pcode__t_integer));  param->append((pcode__t_integer));  param->append((pcode__t_integer)); 
...
#ifdef CLASS
	CLASS::EVENT_OnDragEnter(Cancel, X, Y, Width, Height);
#endif  
 }
...
void EVENT_OnDragMove(t_boolean *Cancel, t_integer X, t_integer Y, t_integer Width, t_integer Height){ 
...
#ifdef CLASS
	CLASS::EVENT_OnDragMove(Cancel, X, Y, Width, Height);
#endif  
 }*/
