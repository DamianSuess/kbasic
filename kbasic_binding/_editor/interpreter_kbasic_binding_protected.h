...
void EVENT_OnEvent1(t_boolean TextSelected, t_boolean PasteAvailable, t_boolean UndoAvailable, t_boolean RedoAvailable){ 
  if (nEVENT_OnEvent1 != 1){
	QList<int> *param = new QList<int>; 
  param->append((pcode__t_boolean));  param->append((pcode__t_boolean));  param->append((pcode__t_boolean));  param->append((pcode__t_boolean)); 

  if ((nEVENT_OnEvent1 == 2) || (nEVENT_OnEvent1 == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnAction", acGroup, acName, param))){
    nEVENT_OnEvent = 2;

    myInterpreter->pushStack(new interpreter_parameter(TextSelected));
    myInterpreter->pushStack(new interpreter_parameter(PasteAvailable));
    myInterpreter->pushStack(new interpreter_parameter(UndoAvailable));
    myInterpreter->pushStack(new interpreter_parameter(RedoAvailable));

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnAction", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnEvent1 = 1;
}
	CLASS::EVENT_OnEvent1(TextSelected, PasteAvailable, UndoAvailable, RedoAvailable); 
 }


void EVENT_OnCursorPositionChanged(){ 
  ...
  if (kbId) nEVENT_OnCursorPositionChanged = 1;
	...
	CLASS::EVENT_OnCursorPositionChanged(); 
 }
