...
void EVENT_OnCursorPositionChanged(t_integer Old, t_integer New){ 
  if (nEVENT_OnCursorPositionChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append(pcode__t_integer);  param->append(pcode__t_integer); 
  if ((nEVENT_OnCursorPositionChanged == 2) || (nEVENT_OnCursorPositionChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnCursorPositionChanged", acGroup, acName, param))){
    nEVENT_OnCursorPositionChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(Old));
		myInterpreter->pushStack(new interpreter_parameter(New));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnCursorPositionChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnCursorPositionChanged = 1;
}
	CLASS::EVENT_OnCursorPositionChanged(Old, New);
 }

void EVENT_OnEditingFinished(){ 
  if (nEVENT_OnEditingFinished != 1){
	QList<int> *param = new QList<int>; 
	
  if ((nEVENT_OnEditingFinished == 2) || (nEVENT_OnEditingFinished == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnEditingFinished", acGroup, acName, param))){
    nEVENT_OnEditingFinished = 2;
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnEditingFinished", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnEditingFinished = 1;
}
	CLASS::EVENT_OnEditingFinished(); 
 }

void EVENT_OnReturnPressed(){ 
  if (nEVENT_OnReturnPressed != 1){
	QList<int> *param = new QList<int>; 
	
  if ((nEVENT_OnReturnPressed == 2) || (nEVENT_OnReturnPressed == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnReturnPressed", acGroup, acName, param))){
    nEVENT_OnReturnPressed = 2;
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnReturnPressed", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnReturnPressed = 1;
}
	CLASS::EVENT_OnReturnPressed(); 
 }


void EVENT_OnSelectionChanged(){ 
  if (nEVENT_OnSelectionChanged != 1){
	QList<int> *param = new QList<int>; 
	
  if ((nEVENT_OnSelectionChanged == 2) || (nEVENT_OnSelectionChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnSelectionChanged", acGroup, acName, param))){
    nEVENT_OnSelectionChanged = 2;
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnSelectionChanged", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnSelectionChanged = 1;
}
	CLASS::EVENT_OnSelectionChanged(); 
 }


void EVENT_OnTextChanged( const QString &  Text){ 
  if (nEVENT_OnTextChanged != 1){
	QList<int> *param = new QList<int>;
	 param->append(pcode__QString);
  if ((nEVENT_OnTextChanged == 2) || (nEVENT_OnTextChanged == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnTextChanged", acGroup, acName, param))){
    nEVENT_OnTextChanged = 2;
		myInterpreter->pushStack(new interpreter_parameter(Text));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnTextChanged", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnTextChanged = 1;
}
	CLASS::EVENT_OnTextChanged(Text);
 }

void EVENT_OnTextEdited( const QString &  Text){ 
  if (nEVENT_OnTextEdited != 1){
	QList<int> *param = new QList<int>;
	 param->append(pcode__QString);
  if ((nEVENT_OnTextEdited == 2) || (nEVENT_OnTextEdited == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnTextEdited", acGroup, acName, param))){
    nEVENT_OnTextEdited = 2;
		myInterpreter->pushStack(new interpreter_parameter(Text));
		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnTextEdited", acGroup, acName, param);
		delete param; return;
	}
	delete param;
  if (kbId) nEVENT_OnTextEdited = 1;
}
	CLASS::EVENT_OnTextEdited(Text);
 }


int nEVENT_OnEvent;

int nEVENT_OnCursorPositionChanged;
int nEVENT_OnEditingFinished;
int nEVENT_OnReturnPressed;
int nEVENT_OnSelectionChanged;
int nEVENT_OnTextChanged;
int nEVENT_OnTextEdited;
...
{  
  ...
  nEVENT_OnCursorPositionChanged = 0;
  nEVENT_OnEditingFinished = 0;
  nEVENT_OnReturnPressed = 0;
  nEVENT_OnSelectionChanged = 0;
  nEVENT_OnTextChanged = 0;
  nEVENT_OnTextEdited = 0;
