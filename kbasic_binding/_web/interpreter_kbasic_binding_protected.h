...
void EVENT_OnEvent(t_boolean BackwardAvailable, t_boolean ForwardAvailable, QString NewURL){ 
  if (nEVENT_OnEvent != 1){
	QList<int> *param = new QList<int>; 
	 param->append((pcode__t_boolean)); param->append((pcode__t_boolean));  param->append((pcode__QString)); 
	
  if ((nEVENT_OnEvent == 2) || (nEVENT_OnEvent == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnEvent", acGroup, acName, param))){
    nEVENT_OnEvent = 2;
		myInterpreter->pushStack(new interpreter_parameter(BackwardAvailable));
		myInterpreter->pushStack(new interpreter_parameter(ForwardAvailable));
		myInterpreter->pushStack(new interpreter_parameter(NewURL));
  ...
  if (kbId) nEVENT_OnEvent = 1;
	...
	CLASS::EVENT_OnEvent(BackwardAvailable, ForwardAvailable, NewURL); 
 }

void EVENT_OnPage(QString NewPage){ 
  if (nEVENT_OnPage != 1){
	QList<int> *param = new QList<int>; 
	 param->append((pcode__QString)); 
	
  if ((nEVENT_OnPage == 2) || (nEVENT_OnPage == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnPage", acGroup, acName, param))){
    nEVENT_OnPage = 2;
		myInterpreter->pushStack(new interpreter_parameter(NewPage));

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnPage", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnPage = 1;
}
	CLASS::EVENT_OnPage(NewPage); 
 }


void EVENT_OnLinkHovered(const QString & Link, const QString & Title, const QString & TextContent){ 
  if (nEVENT_OnLinkHovered != 1){
	QList<int> *param = new QList<int>; 
	 param->append((pcode__QString)); param->append((pcode__QString)); param->append((pcode__QString)); 
	
  if ((nEVENT_OnLinkHovered == 2) || (nEVENT_OnLinkHovered == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnLinkHovered", acGroup, acName, param))){
    nEVENT_OnLinkHovered = 2;
		myInterpreter->pushStack(new interpreter_parameter(Link));
		myInterpreter->pushStack(new interpreter_parameter(Title));
		myInterpreter->pushStack(new interpreter_parameter(TextContent));

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnLinkHovered", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnLinkHovered = 1;
}
	CLASS::EVENT_OnLinkHovered(Link, Title, TextContent); 
 }


void EVENT_OnLoadFinished(t_boolean Ok){ 
  if (nEVENT_OnLoadFinished != 1){
	QList<int> *param = new QList<int>; 
	 param->append((pcode__t_boolean)); 
	
  if ((nEVENT_OnLoadFinished == 2) || (nEVENT_OnLoadFinished == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnLoadFinished", acGroup, acName, param))){
    nEVENT_OnLoadFinished = 2;
		myInterpreter->pushStack(new interpreter_parameter(Ok));

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnLoadFinished", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnLoadFinished = 1;
}
	CLASS::EVENT_OnLoadFinished(Ok); 
 }

void EVENT_OnLoadProgress(t_integer Progress){ 
  if (nEVENT_OnLoadProgress != 1){
	QList<int> *param = new QList<int>; 
	 param->append((pcode__t_integer)); 
	
  if ((nEVENT_OnLoadProgress == 2) || (nEVENT_OnLoadProgress == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnLoadProgress", acGroup, acName, param))){
    nEVENT_OnLoadProgress = 2;
		myInterpreter->pushStack(new interpreter_parameter(Progress));

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnLoadProgress", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnLoadProgress = 1;
}
	CLASS::EVENT_OnLoadProgress(Progress); 
 }

void EVENT_OnLoadStarted(){ 
  if (nEVENT_OnLoadStarted != 1){
	QList<int> *param = new QList<int>; 
	 
	
  if ((nEVENT_OnLoadStarted == 2) || (nEVENT_OnLoadStarted == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnLoadStarted", acGroup, acName, param))){
    nEVENT_OnLoadStarted = 2;
		

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnLoadStarted", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnLoadStarted = 1;
}
	CLASS::EVENT_OnLoadStarted(); 
 }

void EVENT_OnWindowCloseRequested(){ 
  if (nEVENT_OnWindowCloseRequested != 1){
	QList<int> *param = new QList<int>; 
	 
	
  if ((nEVENT_OnWindowCloseRequested == 2) || (nEVENT_OnWindowCloseRequested == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnWindowCloseRequested", acGroup, acName, param))){
    nEVENT_OnWindowCloseRequested = 2;
		

		EventSource = this; myInterpreter->callEvent(kbScope, kbId, "OnWindowCloseRequested", acGroup, acName, param);
		delete param; return;
	}
  
	delete param;
  if (kbId) nEVENT_OnWindowCloseRequested = 1;
}
	CLASS::EVENT_OnWindowCloseRequested(); 
 }
...
int nEVENT_OnPage;
int nEVENT_OnLinkHovered;
int nEVENT_OnLoadFinished;
int nEVENT_OnLoadProgress;
int nEVENT_OnLoadStarted;
int nEVENT_OnWindowCloseRequested;
...
{  
  ...
  nEVENT_OnPage = 0;
  nEVENT_OnLinkHovered = 0;
  nEVENT_OnLoadFinished = 0;
  nEVENT_OnLoadProgress = 0;
  nEVENT_OnLoadStarted = 0;
  nEVENT_OnWindowCloseRequested = 0;
