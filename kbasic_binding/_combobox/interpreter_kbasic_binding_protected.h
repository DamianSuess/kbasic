...
void EVENT_OnEvent(t_integer Index, const QString & Caption){ 
  if (nEVENT_OnEvent != 1){
	QList<int> *param = new QList<int>; 
  param->append(pcode__t_integer);  param->append(pcode__QString); 
  ...
  if ((nEVENT_OnEvent == 2) || (nEVENT_OnEvent == 0 && myInterpreter && myInterpreter->hasEvent(kbScope, kbId, "OnEvent", acGroup, acName, param))){
    nEVENT_OnEvent = 2;
		...
		myInterpreter->pushStack(new interpreter_parameter(Caption));
  ...
  if (kbId) nEVENT_OnEvent = 1;
	...
	CLASS::EVENT_OnEvent(Index, Caption); 
 }
