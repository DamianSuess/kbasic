...
_replacefilelist::_replacefilelist(QWidget *parent) : QListWidget(parent)
{
  connect(this, SIGNAL(itemActivated(QListWidgetItem *)), this, SLOT(itemActivated(QListWidgetItem *)));

}

void _replacefilelist::itemActivated(QListWidgetItem *item)
{
  ...
  if (s.contains("(") > 0){
    ...
    if (sFile.endsWith(".kbasic_class", Qt::CaseInsensitive)){
      ...
      EXTERN_my_mainwindow->openMenuBar(sFile);
    } else if (sFile.endsWith(".kbasic_toolbar", Qt::CaseInsensitive)){
      EXTERN_my_mainwindow->openToolBar(sFile);
